                ...
                return(
                    <div key = {index} className="commonCard">
                        <Link to={"/detail/"+(method==="favorite"?item.method:method)+"/"+item.id} onClick={scrollTop}>
                            ...
                            {cmpDate(item.release_date?item.release_date:item.first_air_date)&&<p className="commonCardComming"> {t("Coming Soon")}</p>}
                            <p className="commonCardRate"> {Math.floor(item.vote_average*10)/10} </p>
                            <div className="commonCardContent">
                                <p className="commonCardName"> {item.title?item.title:item.name} </p>
                                <p className="commonCardDay"> {item.release_date?item.release_date:item.first_air_date} </p>
                            </div>
                        </Link>
                    </div>
                )
